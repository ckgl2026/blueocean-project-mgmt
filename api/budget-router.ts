    ...
    .query(async ({ input }) => {
      ...
      if (input?.search) {
        return db
          .select()
          .from(projectBudgets)
          .where(like(projectBudgets.projectName, `%${input.search}%`))
    ...
    .query(async ({ input }) => {
      ...
      if (found[0].contractId) {
        const contractsList = await db
          ...
          .limit(1);
        if (contractsList.length > 0) contract = contractsList[0];
      }
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!budgetNo) {
        ...
        const latest = await db
          .select()
          .from(projectBudgets)
          .where(like(projectBudgets.budgetNo, `YS-${yearMonth}-%`))
          .orderBy(projectBudgets.budgetNo);

        let sequence = 1;
        if (latest.length > 0) {
          const lastSeq = parseInt(
            latest[latest.length - 1].budgetNo.split("-")[2]
          );
          sequence = lastSeq + 1;
        }
        budgetNo = `YS-${yearMonth}-${String(sequence).padStart(4, "0")}`;
      }

      const result = await db.insert(projectBudgets).values({
        projectName: input.projectName,
        contractId: input.contractId,
        budgetNo,
        part1Materials: input.part1Materials || [],
        part2Labor: input.part2Labor || {
          workshopFee: 0,
          pmSalary: 0,
          staffSalary: 0,
          otherLabor: 0,
        },
        part3Other: input.part3Other || {
          processingFee: 0,
          fuelFee: 0,
          weldingFee: 0,
          transportFee: 0,
      ...
      return { id: Number(result[0].insertId), budgetNo, success: true };
    }),
